...
import { fetchMemberTasks, fetchUserInfo } from '../../api/member' 
...
async function fetchTaskList() {
    try {
        ...
        const currentOfficerName = userInfoRes.data?.username

        if (!currentOfficerName) {
            ElMessage.error('无法获取您的干事身份信息，请重新登录')
            return
        }
        ...
        ElMessage.error('加载任务列表失败，请检查网络')
    }
}
